...
{
	try {
		file_id = HDFLibrary.Hopen("vg_all_test.hdf",
	...
	try {
		sd_id = HDFLibrary.SDstart("vg_all_test.hdf",
			HDFConstants.DFACC_READ);
...
public boolean test1(  ) 
{
    ...
    */
	...
	try {
		h4toh5.H4toh5vdata(h4toh5id,vdata_id,"/group1",null,1);
    ...
    */
	try {
		...
		System.err.println("h4toh5vgroup exception "+h45e5);
   ...
   /*  Test I.6. Test annotation API. */
	...
	try {
		h4toh5.H4toh5annoobj_label(h4toh5id,"/","/group1", 
	...
	try {
		h4toh5.H4toh5annoobj_desc(h4toh5id,"/","/group1", 
			anno_objref, anno_objtag, null,0);
   ...
   /* Test I.8. Test dimensional scale dataset API. */
	...
	System.out.print("Test API using the default name generation: ");
...
public boolean test2(  ) 
{
   ...
   /* Test II.1. Testing SDS API; choose the SDS object with index 0;
         all attributes and dimensional scales of this SDS will
         be converted. */
	...
	try {
		...
		System.err.println("SDselect failed");
		System.err.println("Test II.II: FAIL");
		test2_cleanup();
		return(false);
	}
   ...
   /* Test II.2. Test GR conversion. */
	...
	try {
		...
		System.err.println("GRselect exception "+he2);
		System.err.println("Test II.2: FAIL");
		test2_cleanup();
		return(false);
	}

	try {
		h4toh5.H4toh5image(h4toh5id,image_id,"/group1u",
			"myimage","/mypalgroup","mypal",1,1);
   ...
   /* Test II.3. Test independent vdata conversion API.
      Since Vdata may be attributes, in order to avoid converting attributes;
      we use hdp to find the reference number of an independent vdata in this 
      file and obtain vdata id later. */
	...
	try {
		...
		System.err.println("VSgetid exception "+he3);
		System.err.println("Test II.3: FAIL");
		test2_cleanup();
		return(false);
	}

	try {
		 h4toh5.H4toh5vdata(h4toh5id,vdata_id,"/group1u",
			"myvdata",1);
	} catch (H45Exception h45e3) {
		System.err.println("h4toh5vdata exception "+h45e3);
		System.err.println("Test II.3: FAIL");
	...
	try {
		HDFLibrary.VSdetach(vdata_id);
	} catch (HDFException he3_1) {
		System.err.println("Test II.3: FAIL after test");
		return(false);
	}

   /* Test II.4. Test partial vgroup API. 
      vgroup itself and vgroup with non-vgroup members will be converted. */

   /* Convert vgroup with an image object. */
	...
	try {
		...
		System.err.println("Vget exception "+he4);
   ...
   /* Test II.5. Test the whole vgroup API. 
     Everything under this vgroup is converted.*/
	try {
		h4toh5.H4toh5advgroup(h4toh5id,vgroup_id,"/gwhole",
			"myallvgroup");
	} catch (H45Exception h45e5) {
		System.err.println("h4toh5vgroup exception "+h45e5);
		System.err.println("Test II.5: FAIL");
		test2_cleanup();
		return(false);
	}
	...
	try {
		...
		System.err.println("Vdetach error ");
		System.err.println("Test II.5: FAIL after test");
		return(false);
	}

   /* Test II.6. Test annotation API. */
	/***** object annotations testing, we have to provide the object reference
	 number and object tag that have annotations associate with it.
	 we will use hdp to check the reference of such object. In this 
	 example, vgroup is used. vgroup tag is 1965. vgroup reference is 4.
	...
	try {
		h4toh5.H4toh5annoobj_label(h4toh5id,"/","/group1",
			anno_objref, anno_objtag, "myobjlabel",0);
	} catch (H45Exception h45e6) {
		System.err.println("h4toh5annoobj_label exception "+h45e6);
		System.err.println("Test II.6: FAIL");
		test2_cleanup();
		return(false);
	}
	...
	try {
		h4toh5.H4toh5annoobj_desc(h4toh5id,"/","/group1",
			anno_objref, anno_objtag,"myobjdesc",0);
	} catch (H45Exception h45e6_1) {
		System.err.println("h4toh5annoobj_desc exception "+h45e6_1);
		System.err.println("Test II.6: FAIL");
		test2_cleanup();
		return(false);
	}

	/* Test II.7. Test palette conversion.
	Note: We only tested default parameters; 
	since we already used a default palette group 
	"/HDF4_PALGROUP" when converting the image; so 
	we made another palette group for this test.*/
	...
	try {
		h4toh5.H4toh5pal(h4toh5id,image_id,"/group1","image",
			"/paltestgroup","myownpal",1,0);
	} catch (H45Exception h45e7) {
		System.err.println("h4toh5pal exception "+h45e7);
		System.err.println("Test II.7: FAIL");
		test2_cleanup();
		return(false);
	}
	...
	try {
		...
		System.err.println("Test II.7: FAIL after test");
		return(false);
	}

   /* Test II.8. Test one dimensional scale dataset API. */
	try {
		h4toh5.H4toh5onedimscale(h4toh5id,sds_id,"/group1",
			"sds","/mydimtestgroup","myowndim_ofindex0",1,1,0);
	} catch (H45Exception h45e8) {
		System.err.println("h4toh5onedimscale exception "+h45e8);
		System.err.println("Test II.8: FAIL");
		test2_cleanup();
		return(false);
	}
   ...
   /* Test II.9. Testing file annotation label and description. */

	try {
		h4toh5.H4toh5annofil_label(h4toh5id,"myfilelabel",0);
	} catch (H45Exception h45e41) {
		System.err.println("h4toh5annofil_lable exception "+h45e41);
		System.err.println("Test II.9: FAIL");
		test2_cleanup();
		return(false);
	}

	try {
		h4toh5.H4toh5annofil_desc(h4toh5id,"myfiledesc",0);
	} catch (H45Exception h45e44) {
		System.err.println("h4toh5annofil_desc exception "+h45e44);
		System.err.println("Test II.9: FAIL");
		test2_cleanup();
		return(false);
	}

	System.out.print("Test API using the user name generation: ");
 	System.out.println("PASS");
	return(true);
}
