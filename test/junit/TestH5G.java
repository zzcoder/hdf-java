...
//    int names_found = 0;
//    try { 
//      names_found = H5.H5Gget_obj_info_all(gid, null, objNames, objTypes, objRefs);
//    } 
//    catch (Throwable err) {
//      try { H5.H5Gclose(gid); } catch (Exception ex) {}
//      err.printStackTrace();
...
//  }
  ...
  public void testH5Gget_obj_info_all_gid2() {
    ...
    assertTrue("number found["+names_found+"] different than expected["+objNames.length+"]",names_found == objNames.length);
    for (int i=0; i<objNames.length; i++) {
System.out.println("Name #"+i+" is :"+objNames[i]);
System.out.println("Addr #"+i+" is :"+objRefs[i]);
      assertNotNull("name #"+i+" does not exist",objNames[i]);
      assertTrue(objNames[i].length()>0);
    }
  }
...
//    int names_found = 0;
//    try { 
//      names_found = H5.H5Gget_obj_info_max(gid, objNames, objTypes, objRefs, groups_max_size);
//    } 
//    catch (Throwable err) {
//      try { H5.H5Gclose(gid); } catch (Exception ex) {}
//      err.printStackTrace();
...
//    assertTrue("number found["+names_found+"] different than expected["+(GROUPS.length-1)+"]",names_found == (GROUPS.length-1));
//    for (int i=0; i<objNames.length; i++) {
//System.out.println("Name #"+i+" is :"+objNames[i]);
//System.out.println("Addr #"+i+" is :"+objRefs[i]);
////      assertNotNull("name #"+i+" does not exist",objNames[i]);
...
//    try { H5.H5Gclose(gid); } catch (Exception ex) {}
//
//    assertTrue("number found["+names_found+"] different than expected["+groups_max_size+"]",names_found == groups_max_size);
//    for (int i=0; i<objNames.length; i++) {
//System.out.println("Name #"+i+" is :"+objNames[i]);
//      assertNotNull("name #"+i+" does not exist",objNames[i]);
//      assertTrue(objNames[i].length()>0);
