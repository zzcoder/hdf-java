...
//  }
  ...
  public void testH5Fget_create_plist_closed() throws HDF5LibraryException, Throwable {
    int fid = -1;
  ...
  @Test(expected=HDF5LibraryException.class)
  public void testH5Fget_access_plist_closed() throws HDF5LibraryException, Throwable {
    int fid = -1;
  ...
  public void testH5Fget_intent_rdonly() {
    ...
    try { 
      fid = H5.H5Fopen(H5_FILE, HDF5Constants.H5F_ACC_RDONLY, HDF5Constants.H5P_DEFAULT); 
    }
    catch (Throwable err) {
      fail("H5.H5Fopen: "+err);
    }
  ...
  public void testH5Fget_obj_ids() {
    ...
    for (int i=0; i<OBJ_TYPES.length; i++) {
      try { 
        count = H5.H5Fget_obj_ids_long(H5fid, OBJ_TYPES[i], max_objs, obj_id_list); 
      }
      catch (Throwable err) {
