...
public class HDFView extends JFrame implements ViewManager, ActionListener,
    ...
    private static final String aboutHDFView = "HDF Viewer, " + "Version "
            + ViewProperties.VERSION + "\n" + "For "
            + System.getProperty("os.name") + "\n\n" + "Copyright " + '\u00a9'
            + " 2006-2010 The HDF Group.\n" + "All rights reserved.";
    ...
    public HDFView(String root, List flist, int width, int height, int x, int y) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            try {
                ...
                try {
                    theClass = ViewProperties.loadExtClass().loadClass(
                            className);
                }
                catch (Exception ex2) {
                    theClass = null;
        ...
        if (theClass != null) {
            try {
                Class[] paramClass = { Class
                        .forName("ncsa.hdf.view.ViewManager") };
    ...
    private void createMainWindow(int width, int height, int x, int y) {
        ...
        try {
            this.setIconImage(((ImageIcon) ViewProperties.getHdfIcon())
                    .getImage());
        }
        catch (Exception ex) {
        }
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        if (cmd.equals("Exit")) {
            ...
            NewFileDialog dialog = new NewFileDialog(this, currentDir, ftype,
            ...
            try {
                ...
                JOptionPane.showMessageDialog(this, ex.getMessage() + "\n"
                        + filename, getTitle(), JOptionPane.ERROR_MESSAGE);
            }
        }
            ...
            if (theFile == null) {
                toolkit.beep();
                JOptionPane.showMessageDialog(this, "Select a file to close",
            ...
            if (frames != null) {
                for (int i = 0; i < frames.length; i++) {
                    HObject obj = (HObject) (((DataView) frames[i])
                            .getDataObject());
                    if (obj == null) {
                        continue;
                    }
            ...
            while (!files.isEmpty()) {
                try {
                    treeView.closeFile((FileFormat) files.get(0));
                }
                catch (Exception ex) {
                }
            }
            ...
            FileConversionDialog dialog = new FileConversionDialog(this,
                    typeFrom, typeTo, currentDir, treeView.getCurrentFiles());
            ...
            if (userOptionDialog.isFontChanged()) {
                Font font = null;
                try {
                    font = new Font(ViewProperties.getFontType(), Font.PLAIN,
            ...
            String str = (String) JOptionPane.showInputDialog(this, msg,
                    "Register a file format", JOptionPane.PLAIN_MESSAGE,
                    ViewProperties.getLargeHdfIcon(), null, null);
            ...
            while (local_enum.hasMoreElements()) {
                ...
                if (theKey.endsWith(key)) {
                    JOptionPane.showMessageDialog(this, "Invalid key: " + key
                            + " is taken.", "Register File Format",
                            JOptionPane.ERROR_MESSAGE);
                    return;
                }

                theClassName = FileFormat.getFileFormat(theKey).getClass()
                        .getName();
                if (theClassName.endsWith(className)) {
                    JOptionPane.showMessageDialog(this,
                            "The file format has already been registered: "
                                    + className, "Register File Format",
                            JOptionPane.ERROR_MESSAGE);
                    return;
                }
            }
            ...
            String theKey = (String) JOptionPane.showInputDialog(this,
                    ...
                    JOptionPane.WARNING_MESSAGE, ViewProperties
                            .getLargeHdfIcon(), keylist.toArray(), null);
            ...
            try {
                ...
                JOptionPane.showMessageDialog(this, ex.getMessage(), "HDFView",
                        JOptionPane.ERROR_MESSAGE, ViewProperties
                                .getLargeHdfIcon());
            }
        }
        else if (cmd.equals("HDF4 library")) {
            FileFormat thefile = FileFormat
                    .getFileFormat(FileFormat.FILE_TYPE_HDF4);
            if (thefile == null) {
                return;
            }

            JOptionPane.showMessageDialog(this, thefile.getLibversion(),
                    "HDFView", JOptionPane.PLAIN_MESSAGE, ViewProperties
                            .getLargeHdfIcon());
        }
            ...
            JOptionPane
                    .showMessageDialog(this, info, "HDFView",
                            JOptionPane.PLAIN_MESSAGE, ViewProperties
                                    .getLargeHdfIcon());
        }
            ...
            JOptionPane
                    .showMessageDialog(this, str, "HDFView",
                            JOptionPane.PLAIN_MESSAGE, ViewProperties
                                    .getLargeHdfIcon());
        }
        else if (cmd.equals("About")) {
            JOptionPane
                    .showMessageDialog(this, aboutHDFView, "HDFView",
                            JOptionPane.PLAIN_MESSAGE, ViewProperties
                                    .getLargeHdfIcon());
        }
    ...
    public void removeDataView(DataView dataView) {
        if (!(dataView instanceof JInternalFrame)) {
            toolkit.beep();
            JOptionPane.showMessageDialog(this,
                    "The dataview is not a JInternalFrame.", getTitle(),
                    JOptionPane.ERROR_MESSAGE);
            return;
        }
    ...
    private void openFromSRB() throws Exception {
        if (ctrSrbFileDialog == null) {
            ...
            try {
                ...
                throw (new InstantiationException(
                        "Cannot construct SRBFileDialog"));
            }
        }
        ...
        if (srbFileDialog == null) {
            try {
                ...
                srbFileDialog = (JDialog) ctrSrbFileDialog
                        .newInstance(paramObj);
            }
            catch (Exception ex) {
                throw ex;
            }
    ...
    private static final void loadExtModules() throws Exception {
        ...
        if (extClassLoader == null || moduleListTreeView == null
                || moduleListImageView == null)
            return;

        String[] extTreeViews = { "ext.erdc.TreeViewERDC",
        ...
        for (i = 0; i < args.length; i++) {
            if ("-root".equalsIgnoreCase(args[i])) {
                try {
                    ...
                    || "-geometry".equalsIgnoreCase(args[i])) {
                ...
                String info = "Compiled at " + JAVA_COMPILER + "\nRunning at "
                        + System.getProperty("java.vm.version");
                JOptionPane.showMessageDialog(new JFrame(), info, "HDFView",
                        JOptionPane.PLAIN_MESSAGE, ViewProperties
                                .getLargeHdfIcon());
                System.exit(0);
            }
