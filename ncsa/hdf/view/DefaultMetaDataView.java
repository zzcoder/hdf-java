 ...
 */
public class DefaultMetaDataView extends JDialog implements ActionListener,
        MetaDataView {
    ...
    public Attribute addAttribute(HObject obj) {
        ...
        NewAttributeDialog dialog = new NewAttributeDialog(this, obj, node
        ...
        attrTableModel.fireTableRowsInserted(attrTableModel.getRowCount() - 1,
                attrTableModel.getRowCount() - 1);
        numAttributes++;
    ...
    public Attribute deleteAttribute(HObject obj) {
        ...
        if (idx < 0) {
            JOptionPane.showMessageDialog(getOwner(),
                    "No attribute is selected.", getTitle(),
                    JOptionPane.ERROR_MESSAGE);
            return null;
        }

        int option = JOptionPane.showConfirmDialog(this,
                "Do you want to delete the selected attribute?", getTitle(),
    ...
    private JPanel createGroupInfoPanel(Group g) {
        ...
        if (g.getNumberOfMembersInFile() < ViewProperties.getMaxMembers()) {
            panel
                    .add(new JLabel("Number of members: " + n),
                            BorderLayout.NORTH);
        }
        else {
            panel.add(new JLabel("Number of members: " + n + " (in memory), "
            ...
            if (n > 0) {
                ...
                for (int i = 0; i < n; i++) {
                    ...
                    if (mDims == null) {
                        ...
                        if (isH4
                                && types[i].getDatatypeClass() == Datatype.CLASS_STRING) {
                            rowData[i][2] = String.valueOf(types[i]
                                    .getDatatypeSize());
                        }
                    }
                    else {
    ...
    private JPanel createAttributePanel() {
        ...
        // set cell height for large fonts
        int cellRowHeight = Math.max(16, attrTable.getFontMetrics(
                attrTable.getFont()).getHeight());
    ...
    private JPanel createUserBlockPanel() {
        ...
        String[] displayChoices = { "Text", "Binary", "Octal", "Hexadecimal",
                "Decimal" };
    ...
    private void updateAttributeValue(String newValue, int row, int col) {
        ...
        if (st.countTokens() < array_length) {
            JOptionPane.showMessageDialog(getOwner(),
                    "More data value needed: " + newValue, getTitle(),
                    JOptionPane.ERROR_MESSAGE);
            return;
        }
    ...
    private void writeUserBlock() {
        ...
        if (blkSize1 <= blkSize0) {
            ...
            try {
                ...
                JOptionPane.showMessageDialog(this, "Can't open output file: "
                        + hObject.getFile(), getTitle(),
                        JOptionPane.ERROR_MESSAGE);
                return;
            }
            ...
            JOptionPane.showMessageDialog(this,
                    "Saving user block is successful.", getTitle(),
            ...
            int op = JOptionPane
                    .showConfirmDialog(
                            this,
                            "The user block to write is "
                                    + blkSize1
                                    + " (bytes),\n"
                                    + "which is larger than the user block space in file "
                                    + blkSize0
                                    + " (bytes).\n"
            ...
            if (!outFile.exists()) {
                try {
                    ...
                    JOptionPane.showMessageDialog(this,
                            "Fail to write user block into file. ", getTitle(),
                            JOptionPane.ERROR_MESSAGE);
                    return;
                }
            }
