 ...
 */
public class DefaultTableView extends JInternalFrame implements TableView,
    ...
    private final JCheckBoxMenuItem checkScientificNotation, checkHex,
            checkBin;

    private final DecimalFormat scientificFormat = new DecimalFormat(
            "###.#####E0#");
    ...
    public DefaultTableView(ViewManager theView, HashMap map) {
        ...
        checkScientificNotation = new JCheckBoxMenuItem(
            ...
            if (cmd.equals("Close")) {
                ...
                if ((jtable.getSelectedColumnCount() <= 0)
                    ...
                    JOptionPane.showMessageDialog(this,
                            "Select table cells to write.", "HDFView",
    ...
    private void showLineplot() {
        ...
        if ((rows == null) || (cols == null) || (rows.length <= 0)
            ...
            toolkit.beep();
            JOptionPane.showMessageDialog(this,
                    "Select rows/columns to draw line plot.", getTitle(),
                    JOptionPane.ERROR_MESSAGE);
            return;
        }

        /*
         * Object[] options = { "Column", "Row", "X-axis", "Cancel"}; int option
         * = JOptionPane.showOptionDialog( this,
         * "Do you want to draw line plot by column or row?", getTitle(),
         * JOptionPane.YES_NO_CANCEL_OPTION, JOptionPane.QUESTION_MESSAGE, null,
         * options, options[0]); if (option == 3) return; // cancel the line
         * plot action boolean isRowPlot = (option == 1);
         */
        int nrow = table.getRowCount();
        int ncol = table.getColumnCount();

        LineplotOption lpo = new LineplotOption((JFrame) viewer,
                "Line Plot Options -- " + dataset.getName(), nrow, ncol);
        lpo.setVisible(true);
        ...
        if (isRowPlot) {
            ...
            for (int i = 0; i < nLines; i++) {
                ...
                for (int j = 0; j < cols.length; j++) {
                    try {
                        data[i][j] = Double.parseDouble(table.getValueAt(
                                rows[i], cols[j]).toString());
            ...
            if (xIndex >= 0) {
                ...
                for (int j = 0; j < cols.length; j++) {
                    try {
                        xData[j] = Double.parseDouble(table.getValueAt(xIndex,
                                j).toString());
                    }
                    catch (NumberFormatException ex) {
                        xData[j] = 0;
            ...
            for (int j = 0; j < nLines; j++) {
                ...
                for (int i = 0; i < rows.length; i++) {
                    try {
                        data[j][i] = Double.parseDouble(table.getValueAt(
                                rows[i], cols[j]).toString());
            ...
            if (xIndex >= 0) {
                ...
                for (int j = 0; j < rows.length; j++) {
                    try {
                        xData[j] = Double.parseDouble(table.getValueAt(j,
                                xIndex).toString());
                    }
                    catch (NumberFormatException ex) {
                        xData[j] = 0;
        ...
        if ((yRange[0] == Double.POSITIVE_INFINITY)
            ...
            JOptionPane.showMessageDialog(this,
                    "Cannot show line plot for the selected data. \n"
                            + "Please check the data range.", getTitle(),
                    JOptionPane.ERROR_MESSAGE);
            data = null;
            return;
        }

        Chart cv = new Chart((JFrame) viewer, title, Chart.LINEPLOT, data,
                xData, yRange);
        cv.setLineLabels(lineLabels);

        String cname = dataValue.getClass().getName();
        char dname = cname.charAt(cname.lastIndexOf("[") + 1);
        if ((dname == 'B') || (dname == 'S') || (dname == 'I')
                || (dname == 'J')) {
            cv.setTypeToInteger();
        }
            ...
            public boolean isCellSelected(int row, int column) {
                if ((getSelectedRow() == row)
                    ...
                    cellLabel.setText(String
                            .valueOf(rowStart + row * rowStride)
                    ...
                    while (tokenizer1.hasMoreTokens() && (c < cols)) {
                        ...
                        if (dataset instanceof ScalarDS) {
                            ...
                            while (tokenizer2.hasMoreTokens() && (c < cols)) {
                                updateValueInMemory(tokenizer2.nextToken(), r,
                                        c);
                                c++;
                            }
                        }
    ...
    private void mathConversion() throws Exception {
        ...
        MathConversionDialog dialog = new MathConversionDialog((JFrame) viewer,
                theData);
        dialog.setVisible(true);
        ...
        if (dialog.isConverted()) {
            if (dataset instanceof CompoundDS) {
                Object colData = null;
                try {
                    colData = ((List) dataset.getData()).get(table
                            .getSelectedColumn());
                }
                catch (Exception ex) {
                    ;
                ...
                for (int i = 0; i < rows; i++) {
                    idx_dst = (r0 + i) * w + c0;
                    System
                            .arraycopy(theData, idx_src, dataValue, idx_dst,
                                    cols);
                    idx_src += cols;
                }
            }
    ...
    private class LineplotOption extends JDialog implements ActionListener,
        ...
        public LineplotOption(JFrame owner, String title, int nrow, int ncol) {
            ...
            contentPane.add(new JLabel(" Select plot options:"),
                    BorderLayout.NORTH);
    ...
    public void mouseClicked(MouseEvent e) {
        ...
        if (e.isPopupTrigger()
                ...
                || (System.getProperty("os.name").startsWith("Mac") && (eMod == (InputEvent.BUTTON1_MASK | InputEvent.CTRL_MASK)))) {
            if (popupMenu != null) {
    ...
    private void showObjRefData(long ref) {
        ...
        try {
            ...
            paramObj = new Object[] { dset.getFileFormat(), dset.getName(),
                    dset.getPath() };
        ...
        CompoundBorder border = BorderFactory.createCompoundBorder(
                BorderFactory.createRaisedBevelBorder(), BorderFactory
                        .createTitledBorder(BorderFactory.createLineBorder(
                                ...
                                TitledBorder.RIGHT, TitledBorder.TOP, font,
                                Color.RED));
