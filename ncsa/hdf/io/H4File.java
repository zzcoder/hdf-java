 ...
 */
public class H4File extends DefaultFileFormat
{
    /**
    ...
    /**
     * Checks if the given file is tan HDF4 file.
     * <p>
     ...
     * <p>
     * @return true if the file close is succsessful; otherwise returns false.
     */
    ...
    private MutableTreeNode loadTree()
    {
        ...
        if ( b )
        {
            ...
            for (int i=0; i<n; i++)
            {
                ...
                H4GRImage gr = getGRImage(grid, i, HObject.separator, false);
                if (gr != null)
                {
                    gr.setAccess(grid);
        ...
        if (b)
        {
            ...
            {
                // no duplicate object at top level
                H4SDS sds = getSDS(sdid, i, HObject.separator, false);
                if (sds != null)
                {
                    sds.setAccess(sdid);
    ...
    private void depth_first(MutableTreeNode parentNode)
    {
        ...
        int nelems=0, gid=-1, ref=-1, tag=-1, id=-1, index=-1;
        int[] tags = null;
        int[] refs = null;
        MutableTreeNode node = null;
        ...
        long[] poid = pgroup.getOID();
        int pRef = (int)poid[1];

        try {
            // only read permission is needed to get the file structure
            gid = HDFLibrary.Vattach(fid, pRef, "r");
        } catch (HDFException ex)
        {
            gid = HDFConstants.FAIL;
        }
        ...
        for (int i=0; i<nelems; i++)
        {
            ...
            switch (tag)
            {
                ...
                case HDFConstants.DFTAG_RI8:
                    ...
                    {
                        H4GRImage gr = getGRImage(grid, index, fullPath, true);
                        if (gr != null)
                        {
                            gr.setAccess(grid);
                            node = new DefaultMutableTreeNode(gr);
                ...
                case HDFConstants.DFTAG_NDG:
                    ...
                    if (index != HDFConstants.FAIL)
                    {
                        H4SDS sds = getSDS(sdid, index, fullPath, true);
                        if (sds != null)
                        {
                            sds.setAccess(sdid);
                            node = new DefaultMutableTreeNode(sds);
                ...
                case HDFConstants.DFTAG_VG:
                    ...
                    if (vgroup != null)
                    {
                        ...
                        while (theNode != null && !looped)
                        {
                            ...
                            long[] oid = theGroup.getOID();
                            if (tag == oid[0] && ref == oid[1])
                                looped = true;
                            else
     ...
     * <p>
     * @param grid the GR image idendifier.
     ...
     */
    private final H4GRImage getGRImage(int grid, int index,
        String path, boolean copyAllowed)
    {
        int id=-1, ref=-1;
        H4GRImage gr = null;
        String[] objName = {""};
     ...
     */
    private final H4SDS getSDS(int sdid, int index, String path, boolean copyAllowed)
    {
        int id=-1, ref=-1;
        H4SDS sds = null;
        String[] objName = {""};
