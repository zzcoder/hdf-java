...
public class TestH5G extends TestCase
{
    ...
    private final void _createTestFile()
    {
        int fid = -1;
        
        try {
            fid = H5F.H5Fcreate(H5_FILE, H5Fconstant.H5F_ACC_TRUNC, 
                DEFAULT, DEFAULT);
        } catch (Throwable err) {
            ;//err.printStackTrace();
            fail("H5F.H5Fcreate: "+err);
        }
        
        int gid = -1;
        
        for (int i=0; i<GROUPS.length; i++) {
            gid = _createGroup(fid, GROUPS[i]);
            assertTrue(gid > 0);
    ...
    public void testH5Gcreate_anon() {
        ...
        int gid = -1;
        try {
            gid = H5G.H5Gcreate_anon(fid, DEFAULT, DEFAULT);
        } catch (Throwable err) {
            try { H5F.H5Fclose(fid); } catch (Exception ex) {}
            fail("H5G.H5Gcreate_anon: "+err);
        }
        assertTrue(gid > 0);
    ...
    public void testH5Gget_create_plist() {
        ...
        for (int i=0; i<GROUPS.length; i++) {
            ...
            try { pid = H5G.H5Gget_create_plist(gid); }
            catch (Throwable err) {
                try { H5F.H5Fclose(fid); } catch (Exception ex) {}
                fail("H5G.H5Gget_create_plist: "+err);
            }
            assertTrue(pid>0);
    ...
    public void testH5Gget_info() {
        ...
        try { info = H5G.H5Gget_info(gid); } 
        catch (Throwable err) {
            try { H5F.H5Fclose(fid); } catch (Exception ex) {}
            fail("H5G.H5Gget_info: "+err);
        }
        assertNotNull(info);
    ...
    public void testH5Gget_info_by_name() {
        ...
        try { info = H5G.H5Gget_info_by_name(fid, GROUPS[0], DEFAULT); } 
        catch (Throwable err) {
            fail("H5G.H5Gget_info_by_name: "+err);
        }
        assertNotNull(info);
    ...
    public void testH5Gget_info_by_idx() {
        ...
        try { info = H5G.H5Gget_info_by_idx(fid, GROUPS[0], H5_INDEX.NAME, H5_ITER.INC, 1, DEFAULT); } 
        catch (Throwable err) {
            try { H5F.H5Fclose(fid); } catch (Exception ex) {}
            fail("H5G.H5Gget_info_by_idx: "+err);
        }
        assertNotNull(info);
    ...
    public void testH5Gget_obj_info_all() {
        ...
        try { H5G.H5Gget_obj_info_all(fid, GROUPS[0], objNames, objTypes); } 
        catch (Throwable err) {
            fail("H5G.H5Gget_obj_info_all: "+err);
        }
