...
public class H5CompoundDSTest extends TestCase {
    ...
    public final void testReadByRow() {
        ...
        for (int loop = 0; loop < NLOOPS; loop++) {
            ...
            for (int i = 0; i < nrows; i++) {
                ...
                assertEquals(H5TestFile.DATA_FLOAT[idx], floats[0],
                        Float.MIN_VALUE);
    ...
    public final void testReadBytes() {
        ...
        final int expected = H5TestFile.DIM_SIZE
                * (4 + 4 + H5TestFile.STR_LEN + 4);
    ...
    /**
     * Test method for
     * {@link ncsa.hdf.object.h5.H5CompoundDS#write(java.lang.Object)}.
     * <p>
     * What to test:
     * <ul>
        ...
        for (int loop = 0; loop < NLOOPS; loop++) {
            ...
            for (int i = 0; i < nmembers; i++) {
                ...
                case 1:
                    ...
                    assertNotNull(floats);
                    for (int j = 0; j < H5TestFile.DIM_SIZE; j++) {
                        assertEquals(TEST_VALUE_FLOAT, floats[j],
                                Float.MIN_VALUE);
                    }
                    break;
                case 2:
    ...
    public final void testH5CompoundDSFileFormatStringString() {
        ...
        final H5CompoundDS dset = new H5CompoundDS(file, "NO_SUCH_DATASET",
                "NO_SUCH_PATH");
        dset.init();
        dset.clearData();
        data = null;
    ...
    /**
     * Test method for
     * {@link ncsa.hdf.object.h5.H5CompoundDS#writeMetadata(java.lang.Object)}.
     * <p>
     * What to test:
     * <ul>
    ...
    /**
     * Test method for
     * {@link ncsa.hdf.object.h5.H5CompoundDS#removeMetadata(java.lang.Object)}.
     * <p>
     * What to test:
     * <ul>
     ...
     * .
     * <p>
     * Create a simple compound dataset, i.e. compound members can be either a
     * scalar data or 1D array.
     * 
     * <pre>
    ...
    public final void testCreateStringGroupLongArrayStringArrayDatatypeArrayIntArrayObject() {
        ...
        final float[] expectedFloats = { .1f, .2f, .3f, .4f, .5f, .6f, .7f,
                .8f, .9f, .10f };
        final String[] expectedStr = { "Str 1", "Str 2", "Str 3", "Str 4",
                "Str 5" };
        final long[] dims = { 5 };
        ...
        try {
            dset = (H5CompoundDS) H5CompoundDS.create(compIntName, rootGrp,
                    dims, new String[] { "int" }, new H5Datatype[] { typeInt },
                    memberOrders, compData);
        }
        catch (final Exception ex) {
        ...
        try {
            dset = (H5CompoundDS) H5CompoundDS.create(compFloatName, rootGrp,
                    dims, new String[] { "float" },
                    new H5Datatype[] { typeFloat }, memberOrders, compData);
        }
        catch (final Exception ex) {
            fail("H5CompoundDS.create() failed. " + ex);
        }
        ...
        try {
            dset = (H5CompoundDS) H5CompoundDS.create(compStrName, rootGrp,
                    dims, new String[] { "Str" }, new H5Datatype[] { typeStr },
                    new int[] { 1 }, compData);
        }
        catch (final Exception ex) {
            fail("H5CompoundDS.create() failed. " + ex);
        ...
        compData.add(expectedStr);
        try {
            dset = (H5CompoundDS) H5CompoundDS.create(compIntFloatStrName,
                    rootGrp, dims, new String[] { "int", "float", "Str" },
                    new H5Datatype[] { typeInt, typeFloat, typeStr },
                    new int[] { 2, 2, 1 }, compData);
        }
        catch (final Exception ex) {
            fail("H5CompoundDS.create() failed. " + ex);
        }
     ...
     * .
     * <p>
     * Create a simple compound dataset, i.e. compound members can be
    ...
    public final void testCreateStringGroupLongArrayStringArrayDatatypeArrayIntArrayIntArrayArrayObject() {
        ...
        compData.add(expectedFloats);
        try {
            dset = (H5CompoundDS) H5CompoundDS.create(compName, rootGrp, dims,
                    new String[] { "int", "float" }, new H5Datatype[] {
                            typeInt, typeFloat }, memberRanks, memberDims,
                    compData);
        }
        catch (final Exception ex) {
            ex.printStackTrace();
     ...
     * .
     * <p>
     * Create a simple compound dataset with compression options, i.e. compound
     * members can be multiple-dimensional array.
     * 
     * <pre>
     * public static Dataset create(
     *             String name,
    ...
    public final void testCreateStringGroupLongArrayLongArrayLongArrayIntStringArrayDatatypeArrayIntArrayIntArrayArrayObject() {
        ...
        try {
            dset = (H5CompoundDS) H5CompoundDS.create(compName, rootGrp,
                    H5TestFile.DIMs, maxdims, H5TestFile.CHUNKs, 9,
                    new String[] { "int", "float", "str" }, new H5Datatype[] {
                            typeInt, typeFloat, typeStr },
                    new int[] { 1, 1, 1 },
                    new long[][] { { 1 }, { 1 }, { 1 } }, compData);
        }
        catch (final Exception ex) {
            fail("H5CompoundDS.create() failed. " + ex);
        }
