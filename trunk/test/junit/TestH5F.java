...
public class TestH5F {
    ...
    private final void _testH5Fcreate(String filename, int flag) {
        ...
        try {
            fid = H5.H5Fcreate(filename, flag, 
                HDF5Constants.H5P_DEFAULT, HDF5Constants.H5P_DEFAULT);
        } 
        catch (Throwable err) {
    ...
    private final int _openFile(String filename, int flag) {
        ...
        try { 
            fid = H5.H5Fopen(filename, flag, HDF5Constants.H5P_DEFAULT); 
        }
        catch (Throwable err) {
            fail("H5.H5Fopen: "+err);
        }
    ...
    public void testH5Fget_intent() {
        ...
        fid = _openFile(H5_FILE, HDF5Constants.H5F_ACC_RDWR);
        try { 
        ...
        fid = _openFile(H5_FILE, HDF5Constants.H5F_ACC_RDONLY);
        try { 
            intent = H5.H5Fget_intent(fid); 
        }
        catch (Throwable err) {
    ...
    public void testH5Fget_obj_count() {
        ...
        for (int i=0; i<OBJ_TYPES.length; i++) {
            try { 
                count = H5.H5Fget_obj_count(fid, OBJ_TYPES[i]); 
            }
            catch (Throwable err) {
    ...
    public void testH5Fget_obj_ids() {
        ...
        for (int i=0; i<OBJ_TYPES.length; i++) {
            try { 
                count = H5.H5Fget_obj_ids(fid, OBJ_TYPES[i], max_objs, obj_id_list); 
            }
            catch (Throwable err) {
